...
def _topk(nodes: list[Node], maximize: bool, top_k: int) -> list[Node]:
    ...
    def key(n: Node) -> float:
        # Prefer explicit cv_mean if present, else fall back to valid_metric.
        v = n.cv_mean if n.cv_mean is not None else n.valid_metric
...
def select_final_node_with_info(
    ...
    *,
    ...
    info = PostSearchInfo(
        ...
        n_with_cv=sum(1 for n in candidates if n.cv_mean is not None),
    )

    if selection == "best_valid":
        # Choose best node by metric, excluding buggy nodes
        non_buggy = [n for n in candidates if not n.is_buggy]
        if not non_buggy:
            return None, {**info.to_dict(), "notes": "all_candidates_buggy"}
        return max(non_buggy, key=lambda n: n.metric), info.to_dict()
    ...
    if selection == "last_good":
        ...
        for n in reversed(journal.nodes):
            if n.id not in eligible_ids:
                continue
            if (not only_good) or (not n.is_buggy):
    ...
    if selection == "mean_minus_k_std":
        ...
        for n in candidates:
            ...
            if n.is_buggy:
                continue
            # Require BOTH cv_mean and cv_std - drop nodes missing either
            if n.cv_mean is None or n.cv_std is None:
                continue
            # maximize: lower bound; minimize: upper bound
            robust = n.cv_mean - (k_std * n.cv_std) if maximize else n.cv_mean + (k_std * n.cv_std)
    ...
    if selection == "elite_maximin":
        ...
        cv_means = [n.cv_mean for n in candidates if n.cv_mean is not None and not n.is_buggy]
        if not cv_means:
            return None, {**info.to_dict(), "notes": "no_non_buggy_nodes_with_cv_mean"}

        candidates_with_cv = [n for n in candidates if n.cv_mean is not None and not n.is_buggy]
        ...
        candidates_with_cv.sort(key=lambda n: n.cv_mean, reverse=maximize)
        ...
        best_cv_mean = candidates_with_cv[0].cv_mean
        ...
        if maximize:
            ...
            size_stat = sum(1 for n in candidates_with_cv if n.cv_mean >= threshold)
        else:
            threshold = best_cv_mean + (elite_k_std * std_all)
            size_stat = sum(1 for n in candidates_with_cv if n.cv_mean <= threshold)
